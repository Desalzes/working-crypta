...
from src.indicators import ind_funcs
...
class IndicatorCombinations:
    def __init__(self):
        ...
        self.parameter_ranges = {
            ...
            'volume_composition': {'window': list(range(10, 31)),
        ...
        self.base_indicators = {
            ...
            'volume_composition': self._volume_composition_signal
        }
    ...
    def _get_default_params(self, indicator: str) -> Dict[str, Any]:
        defaults = {
            ...
            'volume_composition': {'window': 20, 'threshold': 1.5}
        }
    ...
    def _process_pair(self, pair_data: Tuple, shared_data: Dict) -> Optional[Tuple]:
        ...
        try:
            ...
            returns = torch.tensor(shared_data['returns']).to(self.device)  # Move to GPU
            ...
            signals1 = torch.tensor(full_signals1).to(self.device)
            signals2 = torch.tensor(full_signals2).to(self.device)
    ...
    def _process_quad(self, quad_data: Tuple, shared_data: Dict, triple_results: Dict) -> Optional[Tuple]:
        """Process a single quad of indicators"""
        ind1, ind2, ind3, ind4 = quad_data
        try:
            # Get parameters from triple results if available, otherwise use defaults
            triple_key = f"{ind1}_{ind2}_{ind3}"
            if triple_key in triple_results:
                params1 = triple_results[triple_key]['parameters'][ind1]
                params2 = triple_results[triple_key]['parameters'][ind2]
                params3 = triple_results[triple_key]['parameters'][ind3]
            else:
                params1 = self._get_default_params(ind1)
                params2 = self._get_default_params(ind2)
                params3 = self._get_default_params(ind3)

            params4 = self._get_default_params(ind4)
            ...
            for i in range(0, len(test_data), self.batch_size):
                ...
                s2 = self.base_indicators[ind2](batch, **params2)
                s3 = self.base_indicators[ind3](batch, **params3)
                s4 = self.base_indicators[ind4](batch, **params4)
                signals.append((torch.tensor(s1) + torch.tensor(s2) +
                                torch.tensor(s3) + torch.tensor(s4)) / 4)

            combined_signals = torch.cat(signals)[:-1]
            success_rate = self._calculate_success_rate(combined_signals, returns)

            return (ind1, ind2, ind3, ind4, success_rate, params1, params2, params3, params4)
        except Exception as e:
            logger.error(f"Error processing quad {ind1}_{ind2}_{ind3}_{ind4}: {e}")
            return None
    ...
    def _volume_composition_signal(self, df: pd.DataFrame, **params) -> np.ndarray:
        """Generate signals from volume composition analysis"""
        ...
        # Volume weighted by price location within candle
        buying_volume = df['volume'] * (df['close'] - df['low']) / (df['high'] - df['low'])
        selling_volume = df['volume'] * (df['high'] - df['close']) / (df['high'] - df['low'])

        # Calculate moving averages of volume components
        buy_vol_ma = buying_volume.rolling(window=window).mean()
        sell_vol_ma = selling_volume.rolling(window=window).mean()

        # Calculate volume ratio
        volume_ratio = buy_vol_ma / sell_vol_ma
        ...
        signals[volume_ratio < (1 / threshold)] = -1

        # Add volume trend confirmation
        volume_trend = df['volume'].rolling(window=window).mean()
        volume_trend_up = df['volume'] > volume_trend

        # Only keep signals when volume confirms the direction
        signals[(signals == 1) & ~volume_trend_up] = 0
        signals[(signals == -1) & ~volume_trend_up] = 0
    ...
    def _normalize_indicator(self, indicator: pd.Series, method: str = 'zscore') -> pd.Series:
        """Normalize indicator values using specified method"""
        if method == 'zscore':
    ...
    async def test_combinations(self, df: pd.DataFrame, lookback_period: int = 100) -> Dict:
        ...
        print("\nTesting indicator pairs...")
        pair_results = self._test_pairs(all_pairs, shared_data)
        results.update(pair_results)

        # Get qualifying pairs
        qualifying_pairs = [(ind1, ind2) for ind1, ind2 in all_pairs
                            if f"{ind1}_{ind2}" in pair_results]

        if qualifying_pairs:
            # Test triples
            print("\nTesting triple combinations...")
            remaining_indicators = set(self.base_indicators.keys()) - set().union(*qualifying_pairs)

            # Generate all possible triples from qualifying pairs
            triples = [(ind1, ind2, ind3)
                       for ind1, ind2 in qualifying_pairs
                       for ind3 in remaining_indicators]

            triple_results = self._test_triples(triples, shared_data, pair_results)
            results.update(triple_results)

            # Get qualifying triples
            qualifying_triples = [(ind1, ind2, ind3) for ind1, ind2, ind3 in triples
                                  if f"{ind1}_{ind2}_{ind3}" in triple_results]

            if qualifying_triples:
                # Test quads
                print("\nTesting quad combinations...")
                remaining_for_quads = set(self.base_indicators.keys()) - set().union(*qualifying_triples)

                # Generate all possible quads from qualifying triples
                quads = [(ind1, ind2, ind3, ind4)
                         for ind1, ind2, ind3 in qualifying_triples
                         for ind4 in remaining_for_quads]

                quad_results = self._test_quads(quads, shared_data, triple_results)
    ...
    def _test_pairs(self, pairs: List[Tuple], shared_data: Dict) -> Dict:
        """Test all indicator pairs"""
        results = {}

        with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
            futures = [executor.submit(self._process_pair, pair, shared_data)
                       for pair in pairs]
    ...
    def _test_triples(self, triples: List[Tuple], shared_data: Dict, pair_results: Dict) -> Dict:
        """Test all indicator triples"""
        results = {}

        with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
            futures = [executor.submit(self._process_triple, triple, shared_data, pair_results)
    ...
    def _test_quads(self, quads: List[Tuple], shared_data: Dict, triple_results: Dict) -> Dict:
        """Test all indicator quads"""
        results = {}
        quad_threshold = 0.48  # Higher threshold for quad combinations

        with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
            futures = [executor.submit(self._process_quad, quad, shared_data, triple_results)
                       for quad in quads]
            ...
            with tqdm(total=len(quads)) as pbar:
                for future in as_completed(futures):
                    result = future.result()
                    if result:
                        ind1, ind2, ind3, ind4, success_rate, params1, params2, params3, params4 = result
                        if success_rate >= quad_threshold:
                            combo_name = f"{ind1}_{ind2}_{ind3}_{ind4}"
                            results[combo_name] = {
    ...
    def _print_results_summary(self, results: Dict):
        ...
        pairs = [k for k in results.keys() if len(k.split('_')) == 2]
        triples = [k for k in results.keys() if len(k.split('_')) == 3]
        quads = [k for k in results.keys() if len(k.split('_')) == 4]
        ...
        top_5 = sorted(results.items(), key=lambda x: x[1]['success_rate'], reverse=True)[:5]
        for combo_name, data in top_5:
            print(f"\n{combo_name}:")
            print(f"  Success Rate: {data['success_rate']:.2f}")
            print("  Parameters:")
            for ind, params in data['parameters'].items():
                print(f"    {ind}: {params}")