...
class Indicators:
    ...
    def _calc_vwap(df: pd.DataFrame) -> Dict:
        ...
        vwap = ind_funcs.calculate_vwap(df)
        current_price = df['close'].iloc[-1]
        
        return {
            'value': float(vwap.iloc[-1]),
            'distance': float(current_price - vwap.iloc[-1]),
            'position': 'above' if current_price > vwap.iloc[-1] else 'below',
            'percent_diff': float((current_price - vwap.iloc[-1]) / vwap.iloc[-1] * 100)
        }
